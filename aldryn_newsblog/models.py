...
class Article(TranslatedAutoSlugifyMixin,
    ...
    def clean(self):
        if self.author is None:
            try:
                self.author = Person.objects.get(user=self.owner)
            except Person.DoesNotExist:
                raise ValidationError({'author': 'The user(%s) who will be the author must have first name and last name.' % self.owner.email})
